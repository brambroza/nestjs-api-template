...
async function main(): Promise<void> {
  ...
  try {
    ...
    const roles: Array<{ id: string; name: string; rules: unknown[] }> = [
      ...
      },
      {
        ...
        name: 'finance-admin',
        rules: [
          { action: 'manage', subject: 'VendorInvoice' },
      ...
      },
      {
        ...
        rules: [
          ...
          { action: 'manage', subject: 'PaymentVoucher' },
          { action: 'read', subject: 'PurchaseOrder' },
          { action: 'read', subject: 'GoodsReceipt' },
          { action: 'read', subject: 'Vendor' },
          { action: 'read', subject: 'Company' },
          { action: 'read', subject: 'Item' },
          { action: 'read', subject: 'TaxCode' },
  ...
  AP: POST /vendor-invoices (3-way match vs PO + GRN) -> /post, payment vouchers withhold WHT and issue WHT certificates
